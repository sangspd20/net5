...
namespace F88.Digital.Application.Helpers
{
    public static class EncryptHelper
    {
        ...
        private const string SecretKeyJwt = "ThisIsSecretKeyForHash";
        private const JwtHashAlgorithm JwtHashAlgorithm = Jwt.JwtHashAlgorithm.HS256;

        public static string EncryptJwt<T>(T item)
        {
            return JsonWebToken.Encode(item, SecretKeyJwt, JwtHashAlgorithm);
        }

        public static T DecryptJwt<T>(string entry)
        {
            try
            {
                var dataStr = JsonWebToken.Decode(entry, SecretKeyJwt);
