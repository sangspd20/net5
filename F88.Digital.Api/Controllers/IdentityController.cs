...
namespace F88.Digital.Api.Controllers
{
    [Route("api/identity")]
    [ApiController]
    public class IdentityController : ControllerBase
    {
        ...
        [AllowAnonymous]
        public async Task<IActionResult> GetTokenAuthorize(string email, string password)
        {
            var ipAddress = GenerateIPAddress();
            var token = await _identityService.GetTokenAuthorize(email,password, ipAddress);
            return Ok(token);
        }
