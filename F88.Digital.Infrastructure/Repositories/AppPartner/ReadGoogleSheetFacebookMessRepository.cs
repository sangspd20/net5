...
{
    public class ReadGoogleSheetFacebookMessRepository : IHostedService, IDisposable
    {
        private bool isRunning = false;
        private readonly ILogger<ReadGoogleSheetFacebookRepository> _logger;
        private Timer _timer;
        private readonly SpreadSheetIdSetting _spreadSheetIdSetting;
        public IServiceProvider Services { get; }
        public ReadGoogleSheetFacebookMessRepository(ILogger<ReadGoogleSheetFacebookRepository> logger, IServiceProvider services, IOptions<SpreadSheetIdSetting> spreadSheetIdSetting)
        {
            _logger = logger;
            Services = services;
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _timer = new Timer(ReadGoogleSheetFacebookMess, null, TimeSpan.Zero,
                TimeSpan.FromSeconds(30));
            return Task.CompletedTask;
        }
        private void ReadGoogleSheetFacebookMess(object state)
        {
            if (!isRunning)
            {
                isRunning = true;
                using (var scope = Services.CreateScope())
                {
                    ...
                    readGoogleSheet.ReadGoogleSheet(_spreadSheetIdSetting.FacebookMessSpreadSheetId);
                }               
                isRunning = false;
            }
