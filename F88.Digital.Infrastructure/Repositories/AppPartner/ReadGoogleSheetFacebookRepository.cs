...
{
    public class ReadGoogleSheetFacebookRepository : IHostedService, IDisposable
    {
        ...
        public ReadGoogleSheetFacebookRepository(ILogger<ReadGoogleSheetFacebookRepository> logger, IServiceProvider services, IOptions<SpreadSheetIdSetting> spreadSheetIdSetting)
        {
            _logger = logger;
            Services = services;
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _timer = new Timer(ReadGoogleSheetFacebook, null, TimeSpan.Zero,
                TimeSpan.FromSeconds(30));
            return Task.CompletedTask;
        }
        private void ReadGoogleSheetFacebook(object state)
        {
            if (!isRunning)
            {
                isRunning = true;
                using (var scope = Services.CreateScope())
                {
                    ...
                    readGoogleSheet.ReadGoogleSheet(_spreadSheetIdSetting.FacebookSpreadSheetId);
                }               
                isRunning = false;
            }
