...
{
    public class SendPolCancelRepository : IHostedService, IDisposable
    {
        private bool isRunning = false;
        private readonly ILogger<SendPolCancelRepository> _logger;
        private Timer _timer;
        public IServiceProvider Services { get; }
        public SendPolCancelRepository(ILogger<SendPolCancelRepository> logger, IServiceProvider services)
        {
            _logger = logger;
            Services = services;
        }
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _timer = new Timer(ReSendPolCancel, null, TimeSpan.Zero,
                TimeSpan.FromMinutes(1));
            return Task.CompletedTask;
        }
        private void ReSendPolCancel(object state)
        {
            if (!isRunning)
            {
                isRunning = true;
                using (var scope = Services.CreateScope())
                {
                    ...
                    reSendPolCancel.JobReSendPolCancel();
                }               
                isRunning = false;
            }
