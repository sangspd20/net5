...
{
    public class ReadGoogleSheetPartnershipRepository : IHostedService, IDisposable
    {
        private bool isRunning = false;
        private readonly ILogger<ReadGoogleSheetPartnershipRepository> _logger;
        private Timer _timer;
        private readonly SpreadSheetIdSetting _spreadSheetIdSetting;
        public IServiceProvider Services { get; }
        public ReadGoogleSheetPartnershipRepository(ILogger<ReadGoogleSheetPartnershipRepository> logger, IServiceProvider services, IOptions<SpreadSheetIdSetting> spreadSheetIdSetting)
        {
            _logger = logger;
            Services = services;
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _timer = new Timer(ReadGoogleSheetPartnership, null, TimeSpan.Zero,
                TimeSpan.FromSeconds(30));
            return Task.CompletedTask;
        }
        private void ReadGoogleSheetPartnership(object state)
        {
            if (!isRunning)
            {
                isRunning = true;
                using (var scope = Services.CreateScope())
                {
                    ...
                    readGoogleSheet.ReadGoogleSheet(_spreadSheetIdSetting.PartnershipSpreadSheetId);
                }               
                isRunning = false;
            }
