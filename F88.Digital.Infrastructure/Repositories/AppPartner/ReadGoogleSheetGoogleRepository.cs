...
{
    public class ReadGoogleSheetGoogleRepository : IHostedService, IDisposable
    {
        ...
        public ReadGoogleSheetGoogleRepository(ILogger<ReadGoogleSheetFacebookRepository> logger, IServiceProvider services, IOptions<SpreadSheetIdSetting> spreadSheetIdSetting)
        {
            _logger = logger;
            Services = services;
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _timer = new Timer(GoogleSheetGoogle, null, TimeSpan.Zero,
                TimeSpan.FromSeconds(30));
            return Task.CompletedTask;
        }
        private void GoogleSheetGoogle(object state)
        {
            if (!isRunning)
            {
                isRunning = true;
                using (var scope = Services.CreateScope())
                {
                    ...
                    readGoogleSheet.ReadGoogleSheet(_spreadSheetIdSetting.GoogleSpreadSheetId);
                }               
                isRunning = false;
            }
