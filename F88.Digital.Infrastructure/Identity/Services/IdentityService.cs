...
using F88.Digital.Application.Exceptions;
...
namespace F88.Digital.Infrastructure.Identity.Services
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<Result<TokenResponse>> GetTokenAuthorize(string email, string password, string ipAddress)
        {
            var user = await _userManager.FindByEmailAsync(email);
            Throw.Exception.IfNull(user, nameof(user), $"No Accounts Registered with {email}.");
            var result = await _signInManager.PasswordSignInAsync(user.UserName, password, false, lockoutOnFailure: false);
            Throw.Exception.IfFalse(user.EmailConfirmed, $"Email is not confirmed for '{email}'.");
            Throw.Exception.IfFalse(user.IsActive, $"Account for '{email}' is not active. Please contact the Administrator.");
            Throw.Exception.IfFalse(result.Succeeded, $"Invalid Credentials for '{email}'.");
            JwtSecurityToken jwtSecurityToken = await GenerateJWToken(user, ipAddress);
            var response = new TokenResponse();
            response.Id = user.Id;
            response.JWToken = new JwtSecurityTokenHandler().WriteToken(jwtSecurityToken);
